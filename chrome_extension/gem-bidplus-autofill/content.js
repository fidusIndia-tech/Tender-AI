(function () {
  ...
  function setExactSearch() {
    ...
    if (current && /exact search/i.test(current.textContent || "")) {
      return true;
    }

    const menuItems = Array.from(document.querySelectorAll(".dropdown-menu a"));
    const exactLink = menuItems.find((link) =>
      /exact search/i.test((link.textContent || "").trim())
    );
  ...
  function runAutofill(bidNumber) {
    ...
    function tryApply() {
      ...
      const searchModeReady = setExactSearch();
